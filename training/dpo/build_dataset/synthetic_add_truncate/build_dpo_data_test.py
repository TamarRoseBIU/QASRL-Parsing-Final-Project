# ==========================================================================
# Test-split counterpart of build_dpo_data.py: same add/truncate corruption over
# test.json, writing dpo_pairs_test.json (used only as the DPO trainer's eval split).
#
...
# ==========================================================================
"""
build_dpo_data_test.py

Constructs (accepted, rejected) pairs for QASRL DPO EVAL from the test set.

============================================================================
HOW THIS DIFFERS FROM build_dpo_data.py (the train/dev version)
============================================================================
This is a same-shaped test.json variant of build_dpo_data.py, for building
a cheap, consistent eval set (validating on test, per the dev->test split
used by Stage_GRPO_Instruct_DEV.py). Two things changed:

  1. Source URL: dev.json -> test.json (same host/path pattern).
  2. Output path: dpo_pairs.json -> dpo_pairs_test.json.

This script itself is otherwise byte-for-byte identical in logic to
build_dpo_data.py -- schema handling, grouping, the 50/50 add/truncate
split, the add-k / donor-sampling / dedup logic, and provenance metadata
are all UNCHANGED. See that file's docstring for the full pipeline
rationale; it's not repeated here.
...
WHAT'S RUN DOWNSTREAM OF THIS SCRIPT'S OUTPUT (dpo_pairs_test.json):
  - The LLM grammar-fix pass (llm_fix_grammar.py) IS run on this file, same
    ...
    its own job -- not because grammar-fixing is being skipped for eval.)
  - The MANUAL review pass (review_tool.html) is skipped for eval data --
    that's a human-in-the-loop step meant for polishing training data by
    hand, and isn't run against test.json. `dpo_pairs_test.grammar_fixed.json`
    (after the LLM-fix job above) is the final eval file used as-is; there's
    no hand-review step after it the way there might be for the train file.
============================================================================
...
TEST_URL = "https://nlp.biu.ac.il/~ron.eliav/qasrl/V-passive_red/test.json"
OUTPUT_PATH = os.environ.get("QASRL_DPO_PAIRS_TEST_OUT",
                             str(pathlib.Path(__file__).resolve().parent / "dpo_pairs_test.json"))
RANDOM_SEED = 42  # same seed as build_dpo_data.py -- fine to reuse since this
                  # draws from a disjoint source file (test.json vs dev.json),
                  # so there's no train/eval leakage risk from sharing it.
...
def download_test_set(url: str = TEST_URL) -> list[dict]:
    log.info(f"Downloading test set from {url} ...")
...
def main():
    ...
    raw_examples = download_test_set()
    ...
    log.info(f"Built {len(records)} DPO eval (test-set) records: {n_add} add "
