...
import paraphrases
print("LOADED FROM:", paraphrases.__file__)
...
# calculates the overlap length between two spans
...
# Finds all candidate matches between system predictions and ground truth,
# cartesian product, Scores each pair using scoring_fn, Keeps only pairs where score ≥ threshold (0.5)
...
#  finds the best one-to-one mapping between predictions and ground truth.
# edges weighted by IoU scores
...
# Builds a graph where nodes are arguments,
# Adds edges between arguments with IoU ≥ threshold,
# Finds connected components, Returns one representative from each component
# Purpose: If a system outputs 3 overlapping wrong predictions, count them as 1 false positive instead of 3.
...
# get_overlap_arguments to find candidate matches
# align_one_to_one to get optimal assignment
# Identifies unmatched predictions and ground truths
# Consolidates unmatched predictions
#
def match_arguments(grt_args: Set[Argument],
    ...
    matched_sys_args = set(sys_to_grt_arg.keys())


    
    unmatched_sys_args = sys_args - matched_sys_args
    unmatched_grt_args = set(grt_args) - set(sys_to_grt_arg.values())
    # this was wrong in the original code.
    # unmatched_grt_args = set(sys_to_grt_arg.values()) - set(grt_args) # false negatives
    # This extension is used to evaluate redundant datasets
    unmatched_sys_args_consolidated = consolidate_by_overlap(unmatched_sys_args, iou, MATCH_IOU_THRESHOLD ) # false positives (grouped)
...
def evaluate(sys_roles: List[Role],
    ...
    # Unlabeled argument evaluation - Match them purely by span overlap
    ...
    # edit by Tamar:
    # n_label_fp += num_invalid_answers
    ...
    for sys_arg, grt_arg in sys_to_grt_arg.items():
        ...
        # # Span matched but wrong label → count as FP and FN
    ...
    # How many distinct ground truth roles (questions) were found?
    # n_unlabel_role_tp = number of unique grt questions that got matched
