...
# The only genuinely boolean columns in the QA-SRL slot schema. Pinning their
# dtype keeps a stray token from silently degrading the column to object, where
# the truthy string "False" would read as True (see paraphrases.py's
# `"NEG" if neg else ""`). Applied to BOTH sides: if only the gold were pinned,
# `q1.is_passive == q2.is_passive` would compare bool to str and never match,
# quietly deflating labelled F1. Unparseable values now raise instead.
QASRL_BOOL_DTYPES = {"is_passive": bool, "is_negated": bool}
...
def main(proposed_path: str, reference_path: str, sents_path=None):
    sys_df = decode_qasrl(pd.read_csv(proposed_path, dtype=QASRL_BOOL_DTYPES))
    grt_df = decode_qasrl(pd.read_csv(reference_path, dtype=QASRL_BOOL_DTYPES))
